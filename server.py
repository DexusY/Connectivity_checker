...
class Server_two:
    def __init__(self, local_ip, port, connection_queue, dev_num, password=b""):
        self.HOST = local_ip
        self.PORT = port
    ...
    def start_server(self):
        ...
        logging.info(f"[{datetime.now()}] Server listening on {self.HOST}:{self.PORT}")
    ...
    def handle_client(self, connection, client_address):
        ...
        logging.info(f"New connection from {client_address} with MAC {mac}")
    ...
    def SetErrorTimeout(self, mac, timeout=60):
        ...
        try:
            ...
            data_in = connection.recv(60)
            received_bytes = struct.unpack("!H", data_in[1:3])[0]
            timeout_set = struct.unpack("!I", data_in[3:7])[0]
            logging.info(f"SetErrorTimeout: received {received_bytes} bytes, timeout set to {timeout_set}s")
        except Exception as e:
            logging.error(f"Failed to set error timeout for device {mac}: {e}")
